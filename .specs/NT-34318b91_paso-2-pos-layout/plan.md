...
- [ ] Build `src/state/CatalogProvider.tsx` (or `hooks/useCatalog.ts`) — manages catalog state keyed by `(branchId, category, q, activeNow, featuredIn)`. Holds a `Map<key, { data: Product[]; pagination; pagesFetched: number; status: 'loading' | 'partial' | 'ready' | 'error' }>` for the lifetime of the session.
- [ ] First-paint fetch: `listProducts({ branchId, category?, q?, activeNow: true, featuredIn?, page: 1, limit: 100 })` blocks initial render of the grid.
- [ ] Background fetch: if `pagination.totalPages > 1`, kick off `page=2..N` requests in parallel (capped to e.g. 5 concurrent) and merge results into state, deduped by `_id`. Skip if any other key is currently active (avoid wasting requests on the user's previous tab).
- [ ] Render order: keep server `sortOrder`; do not re-sort client-side.
- [ ] If the result count after merge exceeds 300 visible tiles, render the grid with `content-visibility: auto` on each tile (`Box sx={{ contentVisibility: 'auto', containIntrinsicSize: '160px 220px' }}`). True virtualization is **not** required.
- [ ] Re-fetch on `branchId` change. Do not re-fetch on tab switch if the cache key already has `status !== 'error'`.

#### Tabs

- [ ] Build `src/components/CategoryTabs.tsx`:
...
- [ ] Parent (`PosHomePage`) translates the tab value into request params: `'all'` → no `category`, no `featuredIn`; `'featured'` → `featuredIn: activeBranchId`, no `category`; any other → `category: <id>`.

#### Search + activeNow toggle

- [ ] Build `src/components/CatalogSearch.tsx`:
...
- [ ] Build `src/components/ActiveNowToggle.tsx`:
...
- [ ] Build `src/components/ProductTile.tsx`:
...
- [ ] Build `src/components/ProductGrid.tsx`:
...
- [ ] Wire `PosHomePage` to compose: header strip (search + activeNow toggle) → `CategoryTabs` → `ProductGrid`. The page owns: `activeTab`, `searchValue`, `activeNow`, and derives the request params + cache key. On tile click, calls the (currently no-op) `CartContext.addItem`.

#### Barcode scanner (HID)

- [ ] Build `src/hooks/useBarcodeScanner.ts`:
...
- [ ] In `PosHomePage`, register `useBarcodeScanner({ onScan: handleScan })`. `handleScan(code)` calls `listProducts({ branchId, q: code, limit: 2 })`:
...
- [ ] Build `src/context/CartContext.tsx`:
...
- [ ] Build `src/components/CartPanel.tsx`:
...
- [ ] Mount `CartProvider` in `PosLayout` (between `WebSocketProvider` and the body). Replace the Phase 2 right-panel placeholder with `<CartPanel />`. On `< md`, `CartPanel` renders inside the bottom `Drawer`.
- [ ] When a tile is tapped on a product with `modifierGroups.some(g => g.required)`, `PosHomePage` also shows a `Snackbar` "Configuración pendiente (Paso 3)" — this is the visible signal that Paso 3 has work to do. Auto-hide 2.5 s.

#### Tests

- [ ] `useBarcodeScanner.test.ts` — fakes `keydown` events with mocked timestamps; verifies burst detection, terminator handling, length threshold, focus filtering, and the `data-scanner-target` override.
- [ ] `CategoryTabs.test.tsx` — mocks `fetch` to return `[ {sortOrder:1,name:'Bebidas'}, {sortOrder:2,name:'Comida'} ]`; asserts the pinned tabs come first and the dynamic tabs are sorted.
- [ ] `CatalogSearch.test.tsx` — types `bur`, advances timers by 150 ms with `vi.useFakeTimers()` + `await act(...)`, asserts the parent's `onSearchChange` fires once with `'bur'`. Clear button resets.
- [ ] `ProductGrid.test.tsx` — given a mocked `listProducts` returning 3 products including one with `modifierGroups: [{ required: true, ...}]`, asserts the `Configurable` chip renders only on that tile.
- [ ] `PosHomePage.test.tsx` (integration) — renders the page inside `<MemoryRouter>` + providers, mocks `fetch` for `/api/categories` and `/api/products`, switches tabs, asserts the URL hit by `listProducts` includes `featuredIn=<branchId>` for `Destacados`, simulates a barcode scan via the `useBarcodeScanner` exposed test seam (or via firing keydown events on `window`), asserts `onProductSelected` was called once for the single-match case, and the `Snackbar` appears for the 0-match case.
- [ ] `CartPanel.test.tsx` — asserts empty state copy, the `$0.00` total, and the disabled `Cobrar (Paso 3)` CTA.

#### Documentation

- [ ] Append a "POS app (frontend-pos)" section to `apps/frontend-pos/README.md` listing the env vars (`VITE_PORT`, `VITE_API_URL`, `VITE_WS_ENDPOINT`), dev port (`:3002`), and the hand-off boundary with Paso 3 (`CartContext.addItem` is a no-op; modifier modal is out of scope; subtotal/total stay at `$0.00`).
- [ ] No edits to `apps/frontend-admin/README.md` (out of scope).
- [ ] No edits to `docs/architecture.md` in this ticket; the long-form cross-app architecture write-up belongs with the runtime-extraction follow-up.
...
- [ ] `pnpm build` succeeds
- [ ] `pnpm test` passes (all Phase 1/2/3 tests)
- [ ] `rg "from '@mui/material" apps/frontend-pos/src` → zero matches
- [ ] `rg "from 'lucide-react" apps/frontend-pos/src` → zero matches
- [ ] `rg "fontSize: [0-9]|fontWeight: [0-9]|borderRadius: [0-9]" apps/frontend-pos/src` → zero matches
- [ ] `rg "console\.log" apps/frontend-pos/src` → zero matches (logger wrapper only)
- [ ] Playwright verification — see QA Plan below (executed by the agent, not the user).
...
- [ ] **Barcode scanner (HID simulation)**
