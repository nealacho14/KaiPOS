...
- [ ] **Evolve existing types** in `packages/shared/src/types/index.ts`:
...
- [ ] **Add new domain types** in `packages/shared/src/types/index.ts`:
...
- [ ] **Update re-exports** in `packages/shared/src/index.ts` — add all new types to the `export type` block
- [ ] **Add collection getters** in `apps/backend/src/db/collections.ts` — add typed getters for `businesses`, `branches`, `categories`, `modifiers`, `tables`, `transactions` (6 new, 3 existing)
- [ ] **Create setup script** at `apps/backend/src/db/setup.ts`:
...
- [ ] **Add seed data function** in the setup script:
...
- [ ] **Add `db:setup` script** to `apps/backend/package.json`:
