...
- **Auth.** JWT passed as `?token=<access_token>` query param on the `$connect` handshake; verified by `src/lib/ws-auth.ts` which also checks the origin shared secret in prod. Unauthenticated connects are rejected at handshake — no DDB row is written.
