...
products.post(
  ...
  async (c) => {
    ...
    const body = (await c.req.json()) as UploadUrlInput;
    const result = await productsService.generateUploadUrl(user, body);
...
products.patch(
  ...
  async (c) => {
    const user = c.get('user')!;
    const id = c.req.param('id');
    const body = updateProductSchema.parse(await c.req.json()) as UpdateProductInput;
...
products.delete(
  ...
  async (c) => {
    const user = c.get('user')!;
    const id = c.req.param('id');
    await productsService.deleteProduct(user, id, {
      route: c.req.path,
      method: c.req.method,
    });
